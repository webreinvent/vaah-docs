<template><div><h1 id="push-notification" tabindex="-1"><a class="header-anchor" href="#push-notification" aria-hidden="true">#</a> Push Notification</h1>
<nav class="table-of-contents"><ul><li><router-link to="#overview">Overview</router-link></li><li><router-link to="#usage">Usage</router-link><ul><li><router-link to="#initializing">Initializing</router-link></li><li><router-link to="#disposing">Disposing</router-link></li><li><router-link to="#asking-permission">Asking Permission</router-link></li><li><router-link to="#subscribing-and-unsubscribing">Subscribing and Unsubscribing</router-link></li><li><router-link to="#pushing-notifications">Pushing Notifications</router-link></li></ul></li><li><router-link to="#best-practices-using-the-pushnotifications-class">Best Practices: Using the PushNotifications Class</router-link></li><li><router-link to="#source-code">Source Code</router-link></li></ul></nav>
<p>The PushNotifications class is responsible for handling both remote and local push notifications in a project. This documentation will guide you through the code and explain its functionalities.</p>
<h2 id="overview" tabindex="-1"><a class="header-anchor" href="#overview" aria-hidden="true">#</a> Overview</h2>
<p>The PushNotifications class is an abstract class that provides static methods to interact with both remote and local notifications. It serves as a wrapper for the RemoteNotifications and LocalNotifications services.</p>
<p>The RemoteNotifications service manages notifications that are sent from a remote source, commonly employed in scenarios similar to chat applications. In contrast, the LocalNotifications service is responsible for handling notifications generated locally on the device, often utilized for user-defined reminders.</p>
<div class="custom-container tip"><p class="custom-container-title">enable notifications</p>
<p>To enable default code from VaahFlutter you need to enable it from env. And to change the service which is used by push notifications; you need to change Push Notifications Service Type from <RouterLink to="/vaahflutter/directory_structure/vaahextendflutter/services/notification/enabling_and_disabling_services.html">env</RouterLink>.</p>
</div>
<h2 id="usage" tabindex="-1"><a class="header-anchor" href="#usage" aria-hidden="true">#</a> Usage</h2>
<p>This section explains how to use the PushNotifications class and its various methods.</p>
<h3 id="initializing" tabindex="-1"><a class="header-anchor" href="#initializing" aria-hidden="true">#</a> Initializing</h3>
<p>To initialize push notifications, call the init method. This method initializes both the remote and local notification services.</p>
<div class="language-dart line-numbers-mode" data-ext="dart"><pre v-pre class="language-dart"><code><span class="token keyword">await</span> <span class="token class-name">PushNotifications</span><span class="token punctuation">.</span><span class="token function">init</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="disposing" tabindex="-1"><a class="header-anchor" href="#disposing" aria-hidden="true">#</a> Disposing</h3>
<p>When you no longer need push notifications, make sure to dispose of the resources by calling the dispose method. This will clean up any ongoing processes and release system resources.</p>
<div class="language-dart line-numbers-mode" data-ext="dart"><pre v-pre class="language-dart"><code><span class="token class-name">PushNotifications</span><span class="token punctuation">.</span><span class="token function">dispose</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
...
<p>To ask the user for permission to display notifications, use the askPermission method. Note that the behavior may vary depending on the configuration of your project.</p>
...
<p>To subscribe to push notifications, call the subscribe method. This method subscribes to both remote and local notifications.</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>To unsubscribe from push notifications, use the unsubscribe method. This method unsubscribes from both remote and local notifications.</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><h3 id="pushing-notifications" tabindex="-1"><a class="header-anchor" href="#pushing-notifications" aria-hidden="true">#</a> Pushing Notifications</h3>
<p>To send a push notification, use the push method. This method takes a PushNotification object as a parameter, along with an optional channel.</p>
<div class="language-dart line-numbers-mode" data-ext="dart"><pre v-pre class="language-dart"><code><span class="token keyword">await</span> <span class="token class-name">PushNotifications</span><span class="token punctuation">.</span><span class="token function">push</span><span class="token punctuation">(</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>The notification parameter is a PushNotification object representing the push notification to be sent. Ensure that the content property of the notification is not empty.</p>
<p>If the notification type is NotificationType.local, the notification will be sent using the LocalNotifications.push method.</p>
<p>For other notification types, the notification will be sent using the RemoteNotifications.push method, along with the specified channel if provided.</p>
<h2 id="best-practices-using-the-pushnotifications-class" tabindex="-1"><a class="header-anchor" href="#best-practices-using-the-pushnotifications-class" aria-hidden="true">#</a> Best Practices: Using the PushNotifications Class</h2>
<p>When implementing push notifications in your Flutter app, it's recommended to use the <code v-pre>PushNotifications</code> class as a central interface for handling both remote and local notifications. This ensures a unified and consistent approach throughout your application. Here are a few reasons why you should use the <code v-pre>PushNotifications</code> class wherever push notifications are needed, rather than accessing the remote and local services directly:</p>
<ol>
<li>
<p>Abstraction and Encapsulation: The <code v-pre>PushNotifications</code> class abstracts away the underlying implementation details of the remote and local notification services. By using this class, developers can work with a high-level interface that provides a consistent set of methods and hides the complexity of interacting with the individual services.</p>
</li>
<li>
<p>Code Modularity and Maintainability: By utilizing the <code v-pre>PushNotifications</code> class, you decouple your app's code from the specific implementation details of the remote and local services. This promotes code modularity and improves maintainability. If there are any changes or updates to the notification services in the future, you can easily make the necessary modifications within the <code v-pre>PushNotifications</code> class without affecting other parts of your app.</p>
</li>
<li>
<p>Simplified Integration: When integrating push notifications into different parts of your app, using the <code v-pre>PushNotifications</code> class provides a straightforward and standardized approach. Developers across your team can easily understand and use the class, resulting in a more consistent implementation of push notifications across your app.</p>
</li>
<li>
<p>Future-Proofing: The <code v-pre>PushNotifications</code> class acts as a layer of abstraction that shields your app from potential changes or updates in the remote and local notification services. If you ever decide to switch to a different notification service or update the existing implementation, you can make the necessary adjustments within the <code v-pre>PushNotifications</code> class, ensuring minimal impact on the rest of your app.</p>
</li>
</ol>
<p>By following these best practices and utilizing the <code v-pre>PushNotifications</code> class, you can achieve a more organized, maintainable, and future-proof implementation of push notifications in your Flutter app. Remember to encourage developers on your team to utilize the <code v-pre>PushNotifications</code> class wherever push notifications are needed and discourage direct usage of the remote and local services.</p>
<div class="custom-container tip"><p class="custom-container-title">configuration</p>
<ul>
<li>As mentioned in <RouterLink to="/vaahflutter/directory_structure/vaahextendflutter/services/notification/enabling_and_disabling_services.html">this file</RouterLink>, you need to change env variables to enable push notifications.</li>
</ul>
</div>
