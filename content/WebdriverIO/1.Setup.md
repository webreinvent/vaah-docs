...
1. Run following commands in the terminal of the Code Editor (Make sure the path is set to your project directory. Refer Step 4 of the Setup).
    ...
    - Now, we need to install various drivers for mobile testing. 
        ...
        ```
        1. `appium-chromium-driver` is used to automate chrome browser on mobile.
        2. `appium-mac2-driver` is used to automate applications on MacOS.
        3. `appium-safari-driver` is used to automate apps on safari browser in iOS device.
        4. `appium-uiautomator2-driver`is used to automate apps on Android device.
        5. `appium-xcuitest-driver` is used to automate apps on iOS device.
...
4. Create a new file in the root folder of the project. (In VSCode, select wdio.conf.js file. Click on the plus icon on the top).
5. Name the file `wdio.env.js`.
6. Add the code given below inside the file:
    ```js
    class Env {
        constructor() {
                this.params = {
    ...
    export default Env;
    ```
7. After this, we need to configure the `wdio.conf.js` file. Open the file and follow the steps mentioned below:
    1. Add the following code at the top of the file to import `wdio.env.js` file created in step 3.
        ```js
    ...
    2. Replace `exports.config` to `export const config`. Refer to the code snippet below:
        ...
        ```
    2. Enter this code: `env: params,` inside exports.config. Refer to the code below:
        ```js
        export const config = {
            env: params,
            //
        ...
        ```
    3. For the `capabilities` replace the code with `capabilities: params.capabilities,`. Refer to the code below:
        ```js
        export const config = {
            ...
        ...
        ```
    4. Similarly replace the baseUrl with `baseUrl: params.base_url,`.
    5. Add the code mentioned below inside "export const config". This adds `appium` service into your project.
        ```js
        export const config= {
            ...
            services: [
        ...
        ```
8. Now, the configuration is completed. But we still need to do some changes with the test files before we can start running our script. Follow the steps mentioned.
    ...
    4. Finally open the file `test.e2e.js` in the directory `test/specs`. Replace the "require" codes with the code snippet below:
        ...
        ```
9. The configuration is now complete. You can start running the test cases now.
