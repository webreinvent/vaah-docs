...
3. Create a new file in the root folder of the project. (In VSCode, select wdio.conf.js file. Click on the plus icon on the top).
...
3. Add the code given below inside the file:
    ...
    export default Env;
    ```
4. After this, we need to configure the `wdio.conf.js` file. Open the file and follow the steps mentioned below:
    ...
    5. Add the code mentioned below inside "export const config". This adds `appium` service into your project.
        ...
        ```
5. Now, the configuration is completed. But we still need to do some changes with the test files before we can start running our script. Follow the steps mentioned.
    1. Change
