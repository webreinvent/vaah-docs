...
2. Go to the text editor for e.g `Php Storm`, on the top nav bar click on `File` > open, a pop screen will be visible to you,   
   choose the directory where you cloned your repo and click on `ok` button.
3. In the `store-dev` repo, you need to create env files
   ...
   Create .env file named as `.env.your-name`  for example `.env.sumit`    
     copy the content from similar env files already existing like `.env.ramesh`
     ::alert{type="warning"}
     NOTE   
     ...
     DB_DATABASE : it should be filled with the database name, you have just create for Vaah Store   
   ...
   APP_URL : https://test.dev.getdemo.dev/store-dev/sumit-k001/public, here replace sumit-k001 with your user-name name   
...
4. Update the `environments` section of the  vaahcms.json file, scroll to the bottom of your left panel and from there     open the file `vaahcms.json`, add the env files you have created like .env.sumit and .env.sumit-feature
...
### Step-7 : 
   ...
   Now you should be to see the latest changes in `Store` Dashboard.
...
- It will show all the `Pipelines` regarding to your `Merge request` then from the latest pipeline which is available   
  at the top in the `Pipeline`. click on pipeline id which is available with hash symbol for e.g #5064
- Click on the #5064 pipeline id, a new screen will appear with various options
- Now click on `Jobs` section, from there details of the Job will be visible to you
   click on job id available with hash symbol for e.g #7789
