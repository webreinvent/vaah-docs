...
::alert{type="warning"}
NOTE   
- First you need to have a git6 account and make sure you have the access of `store-dev`    
...
### Step-6 : Update the develop branch by pull
...
::alert{type="warning"}
Steps 5 and 6 are for Windows users
::
...
- brew install php
- Add below code to `.{zsh/bsh}rc` file   
  export BREW_HOME="/Users/$USER/homebrew"   
  export PHP_HOME="$BREW_HOME/etc/php/8.3"   
  export PATH="$PATH":"$PHP_HOME"
- try running php (php --version), if it gives permission error then run
- `
  chmod +x file_path_for_which_permission isn't there.
  `
- manually install composer: https://getcomposer.org/download/
- cd ~
...
> 2.6.6
- 3. Apache/ Xampp
- download and install https://www.apachefriends.org/download.html
- 4. start server and mysql
- 5. copy the project to htdocs directory of xampp
- 6. open that project in vs code (verify it's located in htdocs)
- open htdocs directory in finder > and change permissions (refer to attached screenshots)
- run
     composer install --ignore-platform-reqs
     composer update --ignore-platform-reqs
7. This will ask some basic questions that we already have discussed while setup the vaahcms.   
   Click on the link to read more about that : https://docs.vaah.dev/vaahcms-2/the-basics/setup
8. After successfully setup the store-dev, login to the Vaah Store by clicking on `backend sign in` button
...
9. After login click on Extends > Modules
...
10. Go to your text editor, scroll to the bottom of your left panel open your custom env file for e.g `.env.sumit` and add `MODULE_STORE_ENV=develop`
11. Run `npm install` and `npm run dev` in the `package.json` file of the `Store` module.
12. Refresh the page or login again by using following url:http://localhost/sumit-k001/store-dev/public/backend/ ,    
