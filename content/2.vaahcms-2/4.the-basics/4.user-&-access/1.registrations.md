...
In VaahCms, the purpose of `Registration` is to prevent `duplicate` entries in the `Users` table.

The `Registration` page will display the all valid `Users` and a form to create and update the `Users` with the following features:
...
#### Create New `User`
- To create a new Registration first click on `+ Create` button and fill all the details of user after it click on `create & new`  button.
