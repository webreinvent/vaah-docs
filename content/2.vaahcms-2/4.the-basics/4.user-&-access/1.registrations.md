...
The `Registration` page will display all registered `Users` and provide a form for creating and updating `Users`, offering the following features:
...
There are two methods to create a registration , Below  we will discuss How to create a registration from `Registrations Admin Pannel`.
...
::
Once the user fills out all the form details and clicks on the 'Create & New' button, a new user will be created, and all the user's details will be saved in the 'vh_registrations' table.
...
Suppose the registration originates from the frontend, and if a user has not yet verified their email, we can use this method to verify the registration. 
If the user is not yet verified, their details will not be added to the Users table".
::
...
such as 10,000 individuals, have registered in `VaahCMS`. Among this substantial user base, only a fraction may be verified users.

The `Registrations` method plays a pivotal role in preventing the `users` table from becoming cluttered with unnecessary front-end registrations.
