...
- If you want to denied permission from your laravel controller so you can use this code
...
### let's take an example for Vue :-
 ...
 #### scenario
 - you have a module `Employee` in that module there is a table `employees` and you want to show edit button if user have permission to edit.
 
 Go to `EmployeesController` =>`getAssets()` function. Add `\Auth::user()->permissions(true);` in `$data['permission']` variable.   
 `\Auth::user()->permissions(true);` this method will get all the permission that user have.
...
```
Now go to your `Employees/components/Table.vue`
