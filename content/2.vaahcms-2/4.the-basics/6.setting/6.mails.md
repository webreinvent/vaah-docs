...
3. Run or setup `cron/daemon` job for 
 ```php
  artisan queue:work --queue=high,medium,low,default --env=env_filename
```
...
To send a generic mail you just need to copy the below code snippet into your controlller.
```php
