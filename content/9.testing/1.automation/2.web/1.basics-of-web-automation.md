...
### forward()
The Forward command causes the browser to traverse one step forwards in the joint session history.
```js
await browser.navigateTo('https://webdriver.io')
await $('=API Reference').click()
await expect(browser).toHaveUrl('https://webdriver.io/docs/api')
...
```
There are many more methods available for browser in wendriverio. Refer to this [documentation](https://webdriver.io/docs/api/browser#methods) to explore more about them.
...
### Browser Matchers
These assetions are related to the browsers.
...
### Element Matchers
These assetions are related to the elements on the page.
...
## Sample script
Below is the sample script which uses many of the functions an concepts explained sections above. The script tries to log into facebook using invalid credential and verifies whether facebook allows us to login or not.
